...
import { PublishCreationModal } from "@/components/publish-creation-modal";
...
export function StudioRightPanel({ generations }: StudioRightPanelProps) {
    ...
    return (
        <div className="w-[320px] h-[calc(100vh-140px)] flex flex-col bg-white/[0.05] backdrop-blur-[20px] border border-white/[0.08] shadow-[0_10px_40px_rgba(0,0,0,0.5)] rounded-[20px] relative z-20 flex-shrink-0 text-zinc-100 overflow-hidden transition-all duration-500">
            ...
            <div className="flex-shrink-0 p-4 border-t border-white/[0.05] bg-white/[0.02]">
                <div className="flex gap-2">
                    ...
                    <Button
                        variant="ghost"
                        size="icon"
                        className="w-9 h-9 bg-white/[0.05] hover:bg-white/[0.1] border border-white/[0.05] rounded-xl text-zinc-300"
                        onClick={() => {
                            const completedIndex = generations.findIndex(g => g.status === 'completed');
                            if (completedIndex >= 0) {
                                setPublishId(generations[completedIndex].id);
                            } else if (generations.length > 0) {
                                setPublishId(generations[0].id);
                            }
                        }}
                    >
            ...
            {publishId && (
                <PublishCreationModal
                    creationId={publishId}

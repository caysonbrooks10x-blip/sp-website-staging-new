...
export function MediaRenderer({ url, altText = "Generated Media", className, fill }: MediaRendererProps) {
    ...
    // Detect if the final URL ends in a common video format
    const isVideo = url.toLowerCase().includes('.mp4') ||
        url.toLowerCase().includes('.webm') ||
        url.toLowerCase().includes('.mov') ||
        url.toLowerCase().includes('video'); // token mappings fallback
    ...
    if (isVideo) {
        return (
            <video
                ...
                autoPlay
                loop
                muted // Mute by default is required by Chrome to autoPlay!
                controls={false}
                playsInline
